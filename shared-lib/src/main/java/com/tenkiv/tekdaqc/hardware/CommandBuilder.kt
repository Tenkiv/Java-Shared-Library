...
fun addAnalogInput(input: AAnalogInput): ABaseQueueVal {
    ...
    val queueValue = QueueValue(
            ...
            Pair(Params.GAIN, Input.gain.ordinal.toByte()),
            Pair(Params.RATE, Input.rate.ordinal.toByte()),
            Pair(Params.BUFFER, Input.bufferState.ordinal.toByte()))
    return queueValue
}
...
fun deactivateAllDigitalInputs(): List<ABaseQueueVal> {
    ...
    for (count in 0..Tekdaqc_RevD.DIGITAL_INPUT_COUNT - 1) {
        val queueValue = QueueValue(
                Commands.REMOVE_DIGITAL_INPUT.ordinalCommandType,
...
fun readSelfGainCalibration(gain: AAnalogInput.Gain, rate: AAnalogInput.Rate, buffer: AnalogInput_RevD.BufferState): ABaseQueueVal {
    val queueValue = QueueValue(
            ...
            Pair(Params.GAIN, gain.ordinal.toByte()),
            Pair(Params.RATE, rate.ordinal.toByte()),
            Pair(Params.BUFFER, buffer.ordinal.toByte()))
    return queueValue
}
